...
namespace Reflection.Tasks
{
    public static class CommonTasks
    {
        ...
        public static IEnumerable<string> GetPublicObsoleteClasses(string assemblyName)
        {
            ...
            var output = assembly.GetTypes().Where(t => t.IsClass && Attribute.GetCustomAttributes(t).Any(x => x is ObsoleteAttribute))
                .Select(x => x.Name);
            return output;
        }
        ...
        {
            string[] namesProperty = propertyPath.Split('.');
            Type type = obj.GetType();
            if (namesProperty.Length == 1)
            {
                return (T)type.GetProperty(namesProperty[0]).GetValue(obj);
            }
            else
            {
                var obj2 = type.GetProperty(namesProperty[0]).GetValue(obj);
                return (T)obj2.GetType().GetProperty(namesProperty[1]).GetValue(obj2);
            }
        }
        ...
        public static void SetPropertyValue(this object obj, string propertyPath, object value)
        {
            ...
            {
                var obj2 = obj.GetType().GetProperty(namesProperty[0]).GetValue(obj);
                obj2.GetType().GetProperty(namesProperty[1]).SetValue(obj2, value);
            }
        }
    }
}
